 ...
 *! maybe just interop with kotlin, alot of logic presented here are defined in
 *! rfc adhoc paper in docs directory, OK this it:
 *!        1. When you start the app, That is a node running, it broadcasts its
 *! presence, if its sees another Node, then sends a ping.
 *!        2. Ask for a permission to connect to the Node, The connection depend
 ...
 *!       3. Share messages(packets) across the network, with DNS like resolver,
...
// routine & also we have a global lock manager struct, idk man. But also lets
// start by atleast freeing this. (Also this is a nice point to write something
...
int main() {
	...
	pthread_create(&broadcast_node_thread_id, &broadcast_attr,
		       (void *)send_udp_broadcast, Nil);

	pthread_create(&confirm_broadcast_discoveries_id, &confirm_disc_attr,
